...
var Core_fasta_aa = [ ];
var Core_fasta_nt = [ ];
var Polymerase_fasta_aa = [ ];
var Polymerase_fasta_nt = [ ];
...
function process_refseqs() {
	...
	write_feature_fasta(Core_fasta_aa, Polymerase_fasta_aa)
}
...
function create_feature_fasta(refseqID, featureID, featureCodons, featureSummary) {
	...
	if (featureID == "Core") {
	  Core_fasta_aa.push(fasta_aa); 
	  Core_fasta_nt.push(fasta_codons);
	
	}	
	
	if (featureID == "Polymerase") {
	  Polymerase_fasta_aa.push(fasta_aa); 
	  Polymerase_fasta_nt.push(fasta_codons);
	}	
	
}

// write feature fasta 
function write_feature_fasta(Core_fasta_aa, Polymerase_fasta_aa) {

	
	var Core_fasta_aa_str = Core_fasta_aa.join("\n");
	glue.command(["file-util", "save-string", Core_fasta_aa_str, "export/Core_fasta.faa"]);
	var Core_fasta_nt_str = Core_fasta_nt.join("\n");
	glue.command(["file-util", "save-string", Core_fasta_nt_str, "export/Core_fasta.fna"]);

	var Polymerase_fasta_aa_str = Polymerase_fasta_aa.join("\n");
	glue.command(["file-util", "save-string", Polymerase_fasta_aa_str, "export/Polymerase_fasta.faa"]);
	var Polymerase_fasta_nt_str = Polymerase_fasta_nt.join("\n");
	glue.command(["file-util", "save-string", Polymerase_fasta_nt_str, "export/Polymerase_fasta.fna"]);
